...
and contextualize AIC retrieval performance. The datasets must be
present in the ../Datasets folder, as generated by the eval_pipeline.py
script.

The user can customize which dataset to analyse and the output name.
Defaults are to analyze everything and save as "dataset_statistics.csv".

Steps:
    1. For each dataset in ../Datasets:
        1.1. Compute number of compounds, number of actives, number
             of inactives and imbalance ratio
        1.2. Repeat step 1.1. for confirmatory assay
        1.3. Select primary hits that have confirmatory readout (referred
             as "valid")
        1.4. Compute number of compounds, number of TPs and FPs, TP and FP
             rate
        1.5. Compute fraction of primary hits that were followed up in
             the confirmatory screen
        1.6. Compute fraction of primary inactives present in the
             confirmatory screen
"""
